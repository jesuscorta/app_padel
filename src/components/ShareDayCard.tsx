      ...
      <div className="overflow-hidden rounded-[32px] border border-[#d9e6dd] bg-white shadow-sm">
        <div className="bg-[#14532d] px-8 py-6 text-white">
          ...
          <div className="mt-3 flex items-end justify-between gap-4">
            ...
            <div className="rounded-full bg-white/12 px-4 py-2 text-sm font-bold">Jornada lista</div>
          </div>
        </div>
        ...
        <div className="space-y-3 p-6">
          {matches.map((match, index) => (
            <div key={`${match.pairA}-${match.pairB}-${index}`} className="rounded-3xl bg-[#f6faf7] p-5">
              ...
              <p className="my-1.5 text-center text-sm font-bold uppercase tracking-[0.2em] text-[#94a3b8]">
                vs
              </p>
              <p className="text-2xl font-black leading-tight text-[#14532d]">{match.pairB}</p>
              {match.scoreLine && (
                <div className="mt-3 rounded-2xl bg-[#14532d]/6 px-4 py-3 text-center text-base font-bold text-[#14532d]">
                  {match.scoreLine}
                </div>
              )}
              <div className="mt-3 rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-[#334155] ring-1 ring-[#e2e8f0]">
